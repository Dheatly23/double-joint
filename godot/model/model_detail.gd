...
func _draw_cube(
	...
	if mask & MASK_FRONT:
		_draw_quad(
			...
			n.z,
			Plane(n.x, 1),
			uv + (Vector2(dim.z, dim.z + dim.y) if uv_unwrap else Vector2(0, dim.y)),
			Vector2(dim.x, -dim.y),
	...
	if mask & MASK_BACK:
		_draw_quad(
			...
			-n.z,
			Plane(-n.x, 1),
			uv + (Vector2(dim.z * 2 + dim.x, dim.z + dim.y) if uv_unwrap else Vector2(0, dim.y)),
			Vector2(dim.x, -dim.y),
			Vector2i(subdiv.x, subdiv.y),
			shsubdiv,
		)
	if mask & MASK_RIGHT:
		_draw_quad(
			...
			-n.x,
			Plane(n.z, -1),
			uv + (Vector2(0, dim.z + dim.y) if uv_unwrap else Vector2(0, dim.y)),
			Vector2(dim.z, -dim.y),
	...
	if mask & MASK_LEFT:
		_draw_quad(
			...
			n.x,
			Plane(-n.z, -1),
			uv + (Vector2(dim.z + dim.x, dim.z + dim.y) if uv_unwrap else Vector2(0, dim.y)),
			Vector2(dim.z, -dim.y),
			Vector2i(subdiv.z, subdiv.y),
			shsubdiv,
		)
	if mask & MASK_UP:
		_draw_quad(
			...
			n.y,
			Plane(n.x, -1),
			uv + (Vector2(dim.z, dim.z) if uv_unwrap else Vector2(0, dim.z)),
			Vector2(dim.x, -dim.z),
	...
	if mask & MASK_DOWN:
		_draw_quad(
			...
			-n.y,
			Plane(n.x, -1),
			uv + Vector2(dim.z + dim.x, 0) if uv_unwrap else uv,
			Vector2(dim.x, dim.z),
			Vector2i(subdiv.x, subdiv.z),
			shsubdiv,
		)
...
func _draw_double_joint(
	...
	_draw_quad(
		...
		t.basis.y,
		Plane(t.basis.x, -1),
		uv + Vector2(dim.z, dim.z) if uv_unwrap else uv + Vector2(0, dim.z),
		Vector2(dim.x, -dim.z),
	)
...
func _draw_finger(
	s: int,
	t: Transform3D,
	finger_pose: Transform3D,
	finger_pose_mid: Transform3D,
	finger_pose_tip: Transform3D,
	uv: Vector2,
	uv_dir: Vector2,
	dim: Vector3,
	mid_rotate_z: bool,
	...
	# Draw Finger
	var t_ := finger_pose
	var r := _get_euler_yxy(t_.basis.orthonormalized())
	r.y = clampf(-r.y, -PI_2, PI_2)
	t_ = Transform3D(t.basis.scaled(Vector3(0.5, 0.5, 0.5)), t * t_.origin)
	_draw_double_joint(0, t_, r, uv, dim, false, false)
	t = Transform3D(
		Basis(Vector3(-1 if right_handed else 1, 0, 0), r.z) *
		Basis(Vector3(0, -1 if right_handed else 1, 0), r.y) *
		Basis(Vector3(-1 if right_handed else 1, 0, 0), r.x) *
		t.basis,
		t_.origin,
	)
	...
	# Draw Finger Middle
	t_ = finger_pose_mid
	var b := t_.basis.orthonormalized()
	var r_ := b.get_euler(EULER_ORDER_ZYX).z if mid_rotate_z else b.get_euler(EULER_ORDER_XYZ).x
	r_ = clampf(r_, -PI_2, PI_2)
	t_ = Transform3D(
		t.basis * (
			Basis(
				Vector3(0, 0, 0.5),
				Vector3(0, 0.5, 0),
				Vector3(-0.5, 0, 0),
			) if mid_rotate_z else Basis.from_scale(Vector3(0.5, 0.5, 0.5))
		),
		t * t_.origin,
	)
	_draw_single_joint(0, t_, r_, uv + uv_dir, dim, false)
	t = Transform3D(
		t.basis * Basis(
			Vector3(0, 0, 1) if mid_rotate_z else Vector3(1, 0, 0),
			r_,
		),
		t_.origin,
	)
...
func rerender():
	...
	_draw_double_joint(0, t_, r, Vector2(32, 48) / 64, Vector3(4, 4, 4) / 64, true, false, 1, subdiv)
	if draw_l2:
		_draw_double_joint(1, t_, r, Vector2(48, 48) / 64, Vector3(4, 4, 4) / 64, true, false, 1 + layer2_offset / 2, subdiv)
	...
	_draw_single_joint(0, t_, r_, Vector2(32, 52) / 64, Vector3(4, 4, 4) / 64, true, 1, subdiv)
	if draw_l2:
		_draw_single_joint(1, t_, r_, Vector2(48, 52) / 64, Vector3(4, 4, 4) / 64, true, 1 + layer2_offset / 2, subdiv)
	t = Transform3D(t.basis * Basis(Vector3(1, 0, 0), r_), t_.origin)

	# Draw Left Hand
	ix = skeleton.find_bone("left_hand")
	t = t * skeleton.get_bone_pose(ix)
	p000 = t * Vector3(-2, -1, -2)
	p001 = t * Vector3(-2, -1, 2)
	p100 = t * Vector3(2, -1, -2)
	p101 = t * Vector3(2, -1, 2)
	_draw_cube(
		0,
		p000,
		p001,
	...
	if draw_l2:
		p000 = t * Vector3(-2 - layer2_offset, -1, -2 - layer2_offset)
		p001 = t * Vector3(-2 - layer2_offset, -1, 2 + layer2_offset)
		p100 = t * Vector3(2 + layer2_offset, -1, -2 - layer2_offset)
		p101 = t * Vector3(2 + layer2_offset, -1, 2 + layer2_offset)
		_draw_cube(
			1,
			p000,
			p001,
			t * Vector3(-2 - layer2_offset, 2, -2 - layer2_offset),
			t * Vector3(-2 - layer2_offset, 2, 2 + layer2_offset),
			p100,
			p101,
			t * Vector3(2 + layer2_offset, 2, -2 - layer2_offset),
	...
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("left_thumb")),
		skeleton.get_bone_pose(skeleton.find_bone("left_thumb_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("left_thumb_tip")),
		Vector2(43, 51) / 64,
		Vector2(-1, 0) / 64,
		Vector3.ONE / 64,
		true,
	...
	# Draw Left Index Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("left_index")),
		skeleton.get_bone_pose(skeleton.find_bone("left_index_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("left_index_tip")),
		Vector2(43, 48) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("left_middle")),
		skeleton.get_bone_pose(skeleton.find_bone("left_middle_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("left_middle_tip")),
		Vector2(42, 48) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	# Draw Left Ring Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("left_ring")),
		skeleton.get_bone_pose(skeleton.find_bone("left_ring_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("left_ring_tip")),
		Vector2(41, 48) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	# Draw Left Pinky Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("left_pinky")),
		skeleton.get_bone_pose(skeleton.find_bone("left_pinky_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("left_pinky_tip")),
		Vector2(40, 48) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	_draw_double_joint(0, t_, r, Vector2(40, 16) / 64, Vector3(4, 4, 4) / 64, true, true, 1, subdiv)
	if draw_l2:
		_draw_double_joint(1, t_, r, Vector2(40, 32) / 64, Vector3(4, 4, 4) / 64, true, true, 1 + layer2_offset / 2, subdiv)
	t.basis = Basis(Vector3(1, 0, 0), -r.z) * Basis(Vector3(0, 1, 0), r.y) * Basis(Vector3(1, 0, 0), -r.x) * t.basis
	...
	t_ = Transform3D(t.basis.scaled(Vector3(2, 2, 2)), t * t_.origin)
	_draw_single_joint(0, t_, r_, Vector2(40, 20) / 64, Vector3(4, 4, 4) / 64, true, 1, subdiv)
	if draw_l2:
		_draw_single_joint(1, t_, r_, Vector2(40, 36) / 64, Vector3(4, 4, 4) / 64, true, 1 + layer2_offset / 2, subdiv)
	t = Transform3D(t.basis * Basis(Vector3(1, 0, 0), r_), t_.origin)
	...
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("right_thumb")),
		skeleton.get_bone_pose(skeleton.find_bone("right_thumb_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("right_thumb_tip")),
		Vector2(48, 19) / 64,
		Vector2(1, 0) / 64,
		Vector3.ONE / 64,
		true,
	...
	# Draw Right Index Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("right_index")),
		skeleton.get_bone_pose(skeleton.find_bone("right_index_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("right_index_tip")),
		Vector2(48, 16) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	# Draw Right Middle Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("right_middle")),
		skeleton.get_bone_pose(skeleton.find_bone("right_middle_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("right_middle_tip")),
		Vector2(49, 16) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	# Draw Right Ring Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("right_ring")),
		skeleton.get_bone_pose(skeleton.find_bone("right_ring_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("right_ring_tip")),
		Vector2(50, 16) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	# Draw Right Pinky Finger
	_draw_finger(
		0,
		t,
		skeleton.get_bone_pose(skeleton.find_bone("right_pinky")),
		skeleton.get_bone_pose(skeleton.find_bone("right_pinky_mid")),
		skeleton.get_bone_pose(skeleton.find_bone("right_pinky_tip")),
		Vector2(51, 16) / 64,
		Vector2(0, 1) / 64,
		Vector3.ONE / 64,
		false,
	...
	t.basis = skeleton.get_bone_global_rest(ix).basis
	t_ = Transform3D(t.basis.scaled(Vector3(2, 2, 2)), t.origin)
	_draw_double_joint(0, t_, r, Vector2(16, 48) / 64, Vector3(4, 4, 4) / 64, true, true, 1, subdiv)
	if draw_l2:
		_draw_double_joint(1, t_, r, Vector2(0, 48) / 64, Vector3(4, 4, 4) / 64, true, true, 1 + layer2_offset / 2, subdiv)
	...
	r_ = clampf(t_.basis.orthonormalized().get_euler(EULER_ORDER_XYZ).x, -PI_2, PI_2)
	t_ = Transform3D(t.basis.scaled(Vector3(2, 2, 2)), t * t_.origin)
	_draw_single_joint(0, t_, r_, Vector2(16, 52) / 64, Vector3(4, 4, 4) / 64, true, 1, subdiv)
	if draw_l2:
		_draw_single_joint(1, t_, r_, Vector2(0, 52) / 64, Vector3(4, 4, 4) / 64, true, 1 + layer2_offset / 2, subdiv)
	t = Transform3D(t.basis * Basis(Vector3(1, 0, 0), r_), t_.origin)
	...
	t.basis = skeleton.get_bone_global_rest(ix).basis
	t_ = Transform3D(t.basis.scaled(Vector3(2, 2, 2)), t.origin)
	_draw_double_joint(0, t_, r, Vector2(0, 16) / 64, Vector3(4, 4, 4) / 64, true, true, 1, subdiv)
	if draw_l2:
		_draw_double_joint(1, t_, r, Vector2(0, 32) / 64, Vector3(4, 4, 4) / 64, true, true, 1 + layer2_offset / 2, subdiv)
	t.basis = Basis(Vector3(0, 1, 0), -r.z) * Basis(Vector3(1, 0, 0), -r.y) * Basis(Vector3(0, 1, 0), -r.x) * t.basis
	...
	r_ = clampf(t_.basis.orthonormalized().get_euler(EULER_ORDER_XYZ).x, -PI_2, PI_2)
	t_ = Transform3D(t.basis.scaled(Vector3(2, 2, 2)), t * t_.origin)
	_draw_single_joint(0, t_, r_, Vector2(0, 20) / 64, Vector3(4, 4, 4) / 64, true, 1, subdiv)
	if draw_l2:
		_draw_single_joint(1, t_, r_, Vector2(0, 36) / 64, Vector3(4, 4, 4) / 64, true, 1 + layer2_offset / 2, subdiv)
	t = Transform3D(t.basis * Basis(Vector3(1, 0, 0), r_), t_.origin)
