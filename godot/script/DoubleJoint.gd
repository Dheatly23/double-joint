...
const PARAM_ROT_Y: String = "rot_y"
...
const SHADER_SCRIPT: Shader = preload("res://shader/DoubleJoint.gdshader")
...
export(float, -180, 180, 0.1) var rot_x: float = 0 setget set_rot_x, get_rot_x
export(float, -90, 90, 0.1) var rot_y: float = 0 setget set_rot_y, get_rot_y

func get_rot_x() -> float:
	return rad2deg(rot_x)

func set_rot_x(v: float):
	v = wrapf(deg2rad(v), -PI, PI)
	rot_x = v
...
func get_rot_y() -> float:
	return rad2deg(rot_y)

func set_rot_y(v: float):
	v = clamp(deg2rad(v), -PI_2, PI_2)
	rot_y = v
	_shader_l.set_shader_param(PARAM_ROT_Y, v)
	_shader_u.set_shader_param(PARAM_ROT_Y, v)
	update_endpoint_node()
...
func update_endpoint_node():
	var basis: Basis = Basis(Vector3(cos(rot_x), 0, sin(rot_x)), -rot_y)
	emit_signal("endpoint_transform_updated", Transform(
		basis, basis * (Vector3.UP * SQRT_1_2)
	))
...
#	mesh = st.commit(mesh)
	...
	_shader_l.set_shader_param(PARAM_ROT_Y, rot_y)
	...
	_shader_u.set_shader_param(PARAM_ROT_Y, rot_y)
